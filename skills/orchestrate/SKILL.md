...
2. **Score and improve the milestone** (inner review-fix loop with external scorer):
   ...
   **If weighted average < threshold**, enter the fix loop:
   ...
   i. If no exit condition triggered, repeat from (e) for the next round.
   ...
   **Fallback**: If Codex scoring fails on every attempt in a round, the orchestrator falls back to self-scoring for that round using the Quality Scoring procedure. This ensures the loop never stalls due to an external tool failure.
